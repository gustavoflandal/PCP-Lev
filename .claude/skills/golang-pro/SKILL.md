...
## Contexto do projeto PCP-Lev

Backend em `backend/`: Go 1.25, Echo v4, pgx/v5 (sem ORM), validator/v10, testify.
Camadas: `cmd/api` (bootstrap), `internal/domain/<agregado>` (regra de negocio), `internal/infra/repository` (pgx), `internal/api/handlers` + `internal/api/middleware` (HTTP), `internal/platform/*` (utilitarios compartilhados).
Siga a nomenclatura em portugues ja usada no codigo (ex.: `fornecedor`, `peca`, `produto`, `usuario`, `saude`).

Responda sempre em portugues do Brasil e mantenha identificadores no idioma ja adotado pelo codigo.
