...
Autenticacao JWT (golang-jwt/v5) com hash bcrypt (golang.org/x/crypto); segredos em `.env` (`JWT_SECRET`).
Revise middleware de auth em `backend/internal/api/middleware` e o fluxo de login do SPA.
